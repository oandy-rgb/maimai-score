import { clamp, expectedAchievementFromRating } from './shared'
...
export function buildNmfResponseFromModel(modelRecord: any, playerKey: string, currentRows: any[]) {
  ...
  const playerRatings = new Map<string, number>(
    Object.entries(payload?.player_ratings ?? {}).map(([key, value]) => [key, Number(value)]),
  )
  ...
  for (const row of currentRows) {
    if (row.player_id === playerKey) currentScores.set(row.song_id, Number(row.achievement))
  }
  ...
  const candidates = songs
    .map((songId) => {
      ...
      const predicted = predict(
        songId,
        expectedAchievementFromRating(playerRatings.get(playerKey) ?? 0, Number(meta.chart_constant)),
      )
      ...
      const factorTotal = songFactorValues.reduce((sum, value) => sum + value, 0) || 1
      const dominantFactor = songFactorValues.reduce((best, value, factor) => (
      ...
      const skillMatch = userFactors[dominantFactor.index] * dominantFactor.value
      const improvement = current == null ? predicted - 97 : predicted - current
    ...
    .filter((entry: any) => entry && entry.sample_count >= 1 && entry.predicted_achievement >= 80)
    .sort((a: any, b: any) => b.recommendation_score - a.recommendation_score)

  const recommendations = candidates
    .filter((entry: any) => entry.sample_count >= 2 && entry.predicted_achievement >= 97)
    .filter((entry: any) => entry.current_achievement == null || entry.predicted_achievement - entry.current_achievement >= -0.5)
    .slice(0, 100)

  const factors = Array.from({ length: songFactors[0]?.length ?? 0 }, (_, factor) => {
